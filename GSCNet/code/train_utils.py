...
class AlternateUpdate:
    def __init__(self, alternate_period, initial_flag=True, flag_random=False): #50,True,True
        self.alternate_period = alternate_period
        self.flag_alternate = initial_flag
        self._counter = 0
        self.flag_random = flag_random
        if self.flag_random:
            self.random_alternate_period = np.random.randint(10, self.alternate_period +1)
        else:
