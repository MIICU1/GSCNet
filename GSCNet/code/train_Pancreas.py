...
from val_3D_2 import var_all_case_LA, var_all_case_Pancrease
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            num_lb = args["labeled_bs"]#2
            num_ulb = batch_size - num_lb #2
            num_sub = int(num_lb / 2) #1
            ...
            with torch.no_grad():
                if not flag_start_self_learning and not args["flag_pseudo_from_student"]:
                    if alternate_indicator.get_alternate_state():
                        ...
                        weighted_outputs_a = weights_1_a.unsqueeze(1) * ema_outputs_soft_1_a + weights_2_a.unsqueeze(
                            1) * ema_outputs_soft_2_a
                        ...
                        weighted_out_a = weights_1_a.unsqueeze(1) * ema_outputs_1_a + weights_2_a.unsqueeze(
                            1) * ema_outputs_2_a

                        pseudo_logits_a, pseudo_outputs_a = torch.max(weighted_outputs_a, dim=1)
                        # plab_a = get_ACDC_masks(pseudo_outputs_a,nms=1)
                        u_bbx1, u_bby1, u_bbz1, u_bbx2, u_bby2, u_bbz2 = rand_bbox_3d(img_lb_a.size(), lam=np.random.beta(4, 4))
                        ...
                        for i in range(img_lb_a.shape[0]):
                            loss_mask_true[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbz1[i]:u_bbz2[i]] = 0
                        loss_mask_true_2 = loss_mask_true.squeeze(1)

                        loss_mask_cat_lb = torch.cat([(1 - loss_mask_true_2), loss_mask_true_2], dim=0)
                        loss_mask_cat_ulb = torch.cat([loss_mask_true_2, (1 - loss_mask_true_2)], dim=0)
                        ...
                        pseudo_logits_a = pseudo_logits_a * loss_mask_true_2 + pseudo_logits_a_one * (
                                    1 - loss_mask_true_2)
                        ...
                        weighted_outputs_b = weights_1_b.unsqueeze(1) * ema_outputs_soft_1_b + weights_2_b.unsqueeze(
                            1) * ema_outputs_soft_2_b
                        ...
                        weighted_out_b = weights_1_b.unsqueeze(1) * ema_outputs_1_b + weights_2_b.unsqueeze(
                            1) * ema_outputs_2_b

                        pseudo_logits_b, pseudo_outputs_b = torch.max(weighted_outputs_b, dim=1)

                        pseudo_logits_b_one = torch.ones_like(pseudo_logits_b)
                        pseudo_logits_b = pseudo_logits_b_one * loss_mask_true_2 + pseudo_logits_b * (
                                    1 - loss_mask_true_2)
                        pseudo_logits_b_numpy = pseudo_logits_b.cpu().numpy()

                        # plab_b = get_ACDC_masks(pseudo_outputs_b,nms=1)
                        net_input_l = img_lb_b * loss_mask_true + img_ulb_b * (1 - loss_mask_true) #CDR_CutMix
                ...
                if alternate_indicator.get_alternate_state() == False:
                    ...
                    if np.random.random() < args.get("cutmix_prob", 1.0):
                        if mtx_bool_conflict is None:
                            img_ulb_s, pseudo_outputs, pseudo_logits = CBR_cut_mix_3d(
                                            img_ulb_w,
                                            pseudo_outputs,
                                            pseudo_logits
                            )
                        else:
                            img_ulb_s, pseudo_outputs, pseudo_logits, mtx_bool_conflict = CBR_cut_mix_3d(
                                            img_ulb_w,
                                            pseudo_outputs,
                                            pseudo_logits,
                                            mtx_bool_conflict
            ...
            img_ulb_s, pseudo_outputs, pseudo_logits = AURS_3d(pseudo_logits, img_ulb_s, out_logits, pseudo_outputs, pseudo_logits, args)
            ...
            if alternate_indicator.get_alternate_state() == False:
                loss_lb_ce = 0
                loss_ulb_ce = ce_loss(pred_ulb_s, pseudo_outputs.long())
            else:
                loss_lb_ce = (ce_loss_none(pred_ulb_s, pseudo_outputs.long())*loss_mask_cat_lb).sum() / (loss_mask_cat_lb.sum() + 1e-16)
                loss_ulb_ce = 0.5 * (ce_loss_none(pred_ulb_s, pseudo_outputs.long()) * loss_mask_cat_ulb).sum() / (loss_mask_cat_ulb.sum() + 1e-16)
            loss_ulb_dice = dice_loss(torch.softmax(pred_ulb_s, dim=1),pseudo_outputs.unsqueeze(1).float())
            loss_ulb = loss_lb_ce + loss_ulb_ce + loss_ulb_dice
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default='config_3d_pan_aut.yml',
    ...
    parser.add_argument('--box_size', type=int, default=24, help='patch_size')
    parser.add_argument('--h_size', type=int, default=4, help='h_size')
    parser.add_argument('--w_size', type=int, default=4, help='w_size')
    parser.add_argument('--d_size', type=int, default=4, help='d_size')
    # top num
