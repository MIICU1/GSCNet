...
from dataloaders.mixaugs import CBR_cut_mix
from dataloaders.dataset_mingyang_1 import (BaseDataSets, TwoStreamBatchSampler, WeakStrongAugment, RandomGenerator)
from networks.net_factory import net_factory
from utils import losses, ramps, AURS_2d
...
def get_ACDC_masks(output, nms=0):
    if nms == 1:
        probs = get_ACDC_2DLargestCC(output)
    return probs
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
#                        II. trainer
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
...
def train(args, pre_snapshot_path, snapshot_path):
    ...
    # pre_trained_model = os.path.join(snapshot_path, '{}_best_stu_model.pth'.format(args["model"]))
    ...
    # alternate params
    alt_flag_epoch_shuffle_teachers = args["alt_flag_epoch_shuffle_teachers"] #False
    alt_flag_conflict_mode = args["alt_flag_conflict_mode"] #"pixel_confidence"
    alt_flag_conflict_stu_use_more = args["alt_flag_conflict_stu_use_more"] #False
    alt_param_ensemble_temp = args["alt_param_ensemble_temp"] #1.0

    alt_param_conflict_weight = args["alt_param_conflict_weight"] #1.0
    alt_param_updating_period_iters = args["alt_param_updating_period_iters"] #50
    alt_flag_updating_period_random = args["alt_flag_updating_period_random"]  #True
    ...
    for epoch_num in iterator:
        ...
        # b) conflict weight
        var_param_conflict_weight = alt_param_conflict_weight #1.0
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            num_lb = args["labeled_bs"] #12
            num_ulb = batch_size - num_lb #12
            num_sub = int(num_lb/2)
            ...
            if alternate_indicator.get_alternate_state():
                img_ulb_a, img_ulb_b = weak_batch[:num_sub], weak_batch[num_sub:num_lb]
                img_lb_a, img_lb_b = weak_batch[num_lb:num_lb+num_sub], weak_batch[num_lb+num_sub:]
                lab_a, lab_b = label_batch[num_lb:num_lb+num_sub], label_batch[num_lb+num_sub:]
            else:
            ...
            with torch.no_grad():
                if not flag_start_self_learning and not args["flag_pseudo_from_student"]:
                    if alternate_indicator.get_alternate_state():
                        ...
                        ema_outputs_1_a = ema_model(img_ulb_a)
                        ema_outputs_soft_1_a = torch.softmax(ema_outputs_1_a, dim=1)
                        ema_outputs_soft_1_a = ema_outputs_soft_1_a.detach()
                        ema_outputs_2_a = ema_model_another(img_ulb_a)
                        ema_outputs_soft_2_a = torch.softmax(ema_outputs_2_a, dim=1)
                        ema_outputs_soft_2_a = ema_outputs_soft_2_a.detach()
                        _, pseudo_outputs_1_a = torch.max(ema_outputs_soft_1_a, dim=1)
                        _, pseudo_outputs_2_a = torch.max(ema_outputs_soft_2_a, dim=1)

                        mtx_bool_conflict_a = pseudo_outputs_1_a != pseudo_outputs_2_a
                        conflict_ratio_a = mtx_bool_conflict_a.float().sum()

                        entropy_1_a = -torch.sum(ema_outputs_soft_1_a * torch.log2(ema_outputs_soft_1_a + 1e-10), dim=1)
                        entropy_2_a = -torch.sum(ema_outputs_soft_2_a * torch.log2(ema_outputs_soft_2_a + 1e-10), dim=1)

                        # weighted sum
                        weights_1_a = torch.exp(-entropy_1_a) / (torch.exp(-entropy_1_a) + torch.exp(-entropy_2_a))
                        weights_2_a = 1.0 - weights_1_a
                        weighted_outputs_a = weights_1_a.unsqueeze(1) * ema_outputs_soft_1_a + weights_2_a.unsqueeze(1) * ema_outputs_soft_2_a
                        weighted_outputs_a = torch.pow(weighted_outputs_a, 1.0 / alt_param_ensemble_temp)
                        weighted_outputs_a = weighted_outputs_a / torch.sum(weighted_outputs_a, dim=1, keepdim=True)

                        weighted_out_a = weights_1_a.unsqueeze(1) * ema_outputs_1_a + weights_2_a.unsqueeze(1) * ema_outputs_2_a

                        pseudo_logits_a, pseudo_outputs_a = torch.max(weighted_outputs_a, dim=1)

                        u_bbx1, u_bby1, u_bbx2, u_bby2 = rand_bbox(img_lb_a.size(), lam=np.random.beta(4, 4))

                        loss_mask_true = torch.ones_like(img_lb_a)  # Create a tensor of ones with the same shape as unlabeled_image
                        for i in range(img_lb_a.shape[0]):
                            loss_mask_true[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]] = 0
                        random_prob = torch.rand(1).item()
                        if random_prob < 0.5:
                            loss_mask_true = loss_mask_true
                        else:
                            loss_mask_true = 1 - loss_mask_true

                        loss_mask_true_2 = loss_mask_true.squeeze(1)
                        loss_mask_cat = torch.cat([(1 - loss_mask_true_2), loss_mask_true_2], dim=0)
                        net_input_unl = img_ulb_a * loss_mask_true + img_lb_a * (1 - loss_mask_true)  #CDR_CutMix
                        pseudo_a = pseudo_outputs_a * loss_mask_true_2 + lab_a * (1 - loss_mask_true_2)
                        pseudo_logits_a_one = torch.ones_like(pseudo_logits_a)
                        pseudo_logits_a = pseudo_logits_a * loss_mask_true_2 + pseudo_logits_a_one * (1 - loss_mask_true_2)

                        ema_outputs_1_b = ema_model(img_ulb_b)
                        ema_outputs_soft_1_b = torch.softmax(ema_outputs_1_b, dim=1)
                        ema_outputs_soft_1_b = ema_outputs_soft_1_b.detach()
                        ema_outputs_2_b = ema_model_another(img_ulb_b)
                        ema_outputs_soft_2_b = torch.softmax(ema_outputs_2_b, dim=1)
                        ema_outputs_soft_2_b = ema_outputs_soft_2_b.detach()
                        _, pseudo_outputs_1_b = torch.max(ema_outputs_soft_1_b, dim=1)
                        _, pseudo_outputs_2_b = torch.max(ema_outputs_soft_2_b, dim=1)
                        mtx_bool_conflict_b = pseudo_outputs_1_b != pseudo_outputs_2_b
                        conflict_ratio_b = mtx_bool_conflict_b.float().sum()
                        entropy_1_b = -torch.sum(ema_outputs_soft_1_b * torch.log2(ema_outputs_soft_1_b + 1e-10), dim=1)
                        entropy_2_b = -torch.sum(ema_outputs_soft_2_b * torch.log2(ema_outputs_soft_2_b + 1e-10), dim=1)
                        # weighted sum
                        weights_1_b = torch.exp(-entropy_1_b) / (torch.exp(-entropy_1_b) + torch.exp(-entropy_2_b))
                        weights_2_b = 1.0 - weights_1_b
                        weighted_outputs_b = weights_1_b.unsqueeze(1) * ema_outputs_soft_1_b + weights_2_b.unsqueeze(1) * ema_outputs_soft_2_b
                        weighted_outputs_b = torch.pow(weighted_outputs_b, 1.0 / alt_param_ensemble_temp)
                        weighted_outputs_b = weighted_outputs_b / torch.sum(weighted_outputs_b, dim=1, keepdim=True)
                        weighted_out_b = weights_1_b.unsqueeze(1) * ema_outputs_1_b + weights_2_b.unsqueeze(1) * ema_outputs_2_b
                        pseudo_logits_b, pseudo_outputs_b = torch.max(weighted_outputs_b, dim=1)
                        pseudo_logits_b_one = torch.ones_like(pseudo_logits_b)
                        pseudo_logits_b = pseudo_logits_b_one * loss_mask_true_2 + pseudo_logits_b * (1 - loss_mask_true_2)
                        net_input_l = img_lb_b * loss_mask_true + img_ulb_b * (1 - loss_mask_true)
                        pseudo_b = lab_b * loss_mask_true_2 + pseudo_outputs_b * (1 - loss_mask_true_2)

                        img_ulb_s = torch.cat([net_input_unl, net_input_l], dim=0)
                        img_lb_w = torch.cat([img_lb_a, img_lb_b], dim=0)
                        img_ulb_w = torch.cat([img_ulb_a, img_ulb_b], dim=0)
                        target_lb = torch.cat([lab_a, lab_b], dim=0)
                        out_logits = torch.cat([weighted_out_a, weighted_out_b], dim=0)
                        mtx_bool_conflict = torch.cat([mtx_bool_conflict_a, mtx_bool_conflict_b], dim=0)
                        pseudo_logits = torch.cat([pseudo_logits_a, pseudo_logits_b], dim=0)
                        pseudo_outputs = torch.cat([pseudo_a, pseudo_b], dim=0)
                        pseudo_outputs = pseudo_outputs.long()

                        conflict_ratio = (conflict_ratio_a+conflict_ratio_b)/num_lb
                    else:
                        ema_model_another.train()
                        ema_model.eval()
                        ema_outputs_1 = ema_model(img_ulb_w)
                        ema_outputs_soft_1 = torch.softmax(ema_outputs_1, dim=1)
                        ema_outputs_soft_1 = ema_outputs_soft_1.detach()
                        ema_outputs_2 = ema_model_another(img_ulb_w)
                        ema_outputs_soft_2 = torch.softmax(ema_outputs_2, dim=1)
                        ...
                        out_logits = weights_1.unsqueeze(1) * ema_outputs_1 + weights_2.unsqueeze(1) * ema_outputs_2
                        # get final outputs
                ...
                if alternate_indicator.get_alternate_state() == False:
                    if np.random.random() < args.get("cutmix_prob", 1.0):
                        if mtx_bool_conflict is None:
                            img_ulb_s, pseudo_outputs, pseudo_logits = CBR_cut_mix(
                                            img_ulb_w,
                                            pseudo_outputs,
                                            pseudo_logits
                            )
                        else:
                            img_ulb_s, pseudo_outputs, pseudo_logits, mtx_bool_conflict = CBR_cut_mix(
                                            img_ulb_w,
                                            pseudo_outputs,
                                            pseudo_logits,
                                            mtx_bool_conflict
            ...
            img_ulb_s, pseudo_outputs, pseudo_logits = AURS_2d(pseudo_logits, img_ulb_s, out_logits, pseudo_outputs,pseudo_logits, args)
            img_ulb_s = img_ulb_s.unsqueeze(1)
            # 4) forward
            ...
            if alternate_indicator.get_alternate_state() == False:
                ...
                loss_ulb_ce = (ce_loss_none(pred_ulb_s, pseudo_outputs.long()) * loss_mask_cat).sum() / (loss_mask_cat.sum() + 1e-16)
            loss_ulb_dice = dice_loss(torch.softmax(pred_ulb_s, dim=1),
                                        pseudo_outputs.unsqueeze(1).float(),
                                        ignore=(pseudo_logits < args["conf_threshold"]).float())
            loss_ulb = loss_ulb_ce + loss_ulb_dice
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default='config_2d_aut_pre_6.yml',
    ...
    parser.add_argument('--exp', type=str,
                        default='ACDC_13/POST-NoT', help='experiment_name')
    parser.add_argument('--exp_2', type=str,
                        default='ACDC_13/POST-NoT', help='experiment_name')
    ...
    parser.add_argument('--box_size', type=int, default=64, help='patch_size')
    ...
    pre_snapshot_path = "{}/{}_{}_labeled/{}".format(
        args["pre_res_path"], args["exp"], args["labeled_num"], args["model"])
    snapshot_path = "{}/{}_{}_labeled/{}".format(
        args["res_path"], args["exp_2"], args["labeled_num"], args["model"])
