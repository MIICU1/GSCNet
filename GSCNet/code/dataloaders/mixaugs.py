...
def CBR_cut_mix(unlabeled_image, unlabeled_mask, unlabeled_logits, unlabeled_conflict=None):
    mix_unlabeled_image = unlabeled_image.clone()
    mix_unlabeled_target = unlabeled_mask.clone()
    mix_unlabeled_logits = unlabeled_logits.clone()
    if unlabeled_conflict is not None:
    ...
    del unlabeled_image, unlabeled_mask, unlabeled_logits
    return mix_unlabeled_image, mix_unlabeled_target, mix_unlabeled_logits
def cut_mix_2(unlabeled_image, unlabeled_mask, unlabeled_logits, cutmix_mask, unlabeled_conflict=None):
    mix_unlabeled_image = unlabeled_image.clone()
    mix_unlabeled_target = unlabeled_mask.clone()
    mix_unlabeled_logits = unlabeled_logits.clone()
    if unlabeled_conflict is not None:
    ...
    # cut & paste
    for i in range(0, mix_unlabeled_image.shape[0]):
        mix_unlabeled_image[i, :, :, :] = unlabeled_image[i, :, :, :]*(1-cutmix_mask) + unlabeled_image[u_rand_index[i], :, :, :]*cutmix_mask
        # label is of 3 dimensions
        #         mix_unlabeled_target[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]] = \
        #             unlabeled_mask[u_rand_index[i], :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]]
        mix_unlabeled_target[i, :, :] = unlabeled_mask[i, :, :] * (1 - cutmix_mask) + unlabeled_mask[u_rand_index[i], :, :] * cutmix_mask

        mix_unlabeled_logits[i, :, :] = unlabeled_logits[i, :, :] * (1 - cutmix_mask) + unlabeled_logits[u_rand_index[i], :, :] * cutmix_mask

        if unlabeled_conflict is not None:
            mix_unlabeled_conflict[i, :, :] = unlabeled_conflict[i, :, :] * (1 - cutmix_mask) + unlabeled_conflict[u_rand_index[i], :,:] * cutmix_mask
...
def CBR_cut_mix_3d(unlabeled_image, unlabeled_mask, unlabeled_logits, unlabeled_conflict=None):
    mix_unlabeled_image = unlabeled_image.clone()
    mix_unlabeled_target = unlabeled_mask.clone()
    mix_unlabeled_logits = unlabeled_logits.clone()
    if unlabeled_conflict is not None:
